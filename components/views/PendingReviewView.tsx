...
const PendingReviewView: React.FC<PendingReviewViewProps> = ({ 
    ...
    const checkAndApprove = () => {
        ...
        if (hasRiskySource) { setShowElevationWarning(true); return; }
        if (sourceNotes.some(n => n.status === 'Draft')) logNotification(workspace, 'info', "Creating canon glossary term from Draft note.");
        handleApprove();
    };
    ...
    const mergeResults = Object.entries(workspace.glossary.index.terms)
        .filter(([id, t]: [string, any]) => t.primaryName.toLowerCase().includes(mergeSearch.toLowerCase()) || t.aliases.some((a: string) => a.toLowerCase().includes(mergeSearch.toLowerCase())))
        .slice(0, 10);
            ...
            <div className="bg-panel border border-border rounded-xl overflow-hidden shadow-2xl mb-6">
                ...
                <div className="p-6 space-y-6">
                    <div className="space-y-2">
                        ...
                        <Input value={proposedName} onChange={e => setProposedName(e.target.value)} className="text-lg font-bold" />
                    </div>
                        ...
                        <div className="space-y-2">
                            ...
                            <div className="bg-surface border border-border rounded-lg p-3 max-h-[150px] overflow-y-auto space-y-2">
                                {pending.detectedSnippets.map((s, i) => (
                                    <div key={i} className="text-xs text-text2 italic border-l-2 border-accent/30 pl-2"><Quote size={10} className="inline mr-1 opacity-50"/>"{s}"</div>
                                ))}
                            </div>
                        </div>
                        <div className="space-y-2">
                            ...
                            <div className="bg-surface border border-border rounded-lg p-3 max-h-[150px] overflow-y-auto space-y-1">
                                {pending.detectedInNoteIds.map(id => (
                                    <button key={id} onClick={() => onOpenNote(id)} className="flex items-center gap-2 text-xs text-accent hover:underline w-full text-left">
                                        <ArrowRight size={12} /> {workspace.notes[id]?.title || id}
                                    </button>
                                ))}
                            </div>
                        </div>
                        ...
                        {mode === 'review' ? (
                            <div className="space-y-4">
                                <div className="space-y-2">
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                        {workspace.settings.universeTags.tags.map(tag => {
                                            ...
                                            return (
                                                <button key={tag} onClick={() => toggleScope(tag)} className={`px-3 py-1 text-xs rounded-full border transition-all ${isSelected ? 'bg-accent text-bg border-accent font-bold' : 'bg-panel border-border text-text2 hover:border-text2'}`}>
                                                    {tag}
                                                </button>
                                            );
                                        })}
                                ...
                                <div className="flex gap-4 pt-4">
                                    <Button onClick={checkAndApprove} className="flex-1 gap-2 py-6 text-base"><Check size={18} /> Approve & Create</Button>
                                    <Button variant="outline" onClick={() => setMode('merge')} className="flex-1 gap-2 py-6 text-base"><Merge size={18} /> Merge as Alias...</Button>
                                </div>
                            </div>
                        ) : (
                            <div className="space-y-4 animate-in slide-in-from-right-4">
                                ...
                                <div className="relative">
                                    ...
                                    <Input autoFocus placeholder="Search existing terms..." className="pl-9" value={mergeSearch} onChange={e => setMergeSearch(e.target.value)} />
                                </div>
                                <div className="border border-border rounded-lg max-h-[200px] overflow-y-auto bg-surface">
                                    {mergeResults.map(([id, t]: [string, any]) => (
                                        <button key={id} onClick={() => handleMerge(id)} className="w-full text-left p-3 hover:bg-accent/10 border-b border-border/50 last:border-0 transition-colors flex items-center justify-between group">
                                            <div><div className="font-bold text-sm text-text group-hover:text-accent">{t.primaryName}</div>{t.aliases.length > 0 && <div className="text-xs text-muted">Aliases: {t.aliases.join(', ')}</div>}</div>
                                            <Merge size={14} className="opacity-0 group-hover:opacity-100 text-accent" />
                                        </button>
                                    ))}
                                    {mergeResults.length === 0 && <div className="p-4 text-center text-xs text-muted italic">No matching terms found.</div>}
                                </div>
                            </div>
                        )}
                    </div>
